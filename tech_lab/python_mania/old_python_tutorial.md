...
`methods or variables start and end with 2 underscores are called `Magic method/variable`.  
They are preserved by python itself.  

`__all__` variable specified which attribute of current module can be imported when doing `from current\_module import * `

    # module name is my_test.py
    a = 'a'
    b = 'b'
    __all__ = ['a']
...
Usually a module can be imported only once, but you can use built-in reload to import it any times you want, it was imported from .pyc by using reload



If you write two file:
vim a.py
from b import b
a = 1
...
run python a.py, you'll get an error, cause this will trigger infinite import, but python only import a module once, so that at the third time, the import would fail


from pprint import pprint
pprint can print list in a pretty way:
similar to      for i in L: print i


Change sys.python search path: export PYTHONPATH=$PYTHONPATH:~/python
