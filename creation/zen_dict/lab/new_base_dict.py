...
def get_dict(first_lan='', second_lan='', transfer=False, update=True):
    """ Using for check if target_dict exists, if not, make one, else, get it. 
        All dict name are represent in English. """
    global __doc__
    first_lan_inquiry = "Please type in the first language of the dict:"

    first_lan = first_lan or raw_input('%s\n' % first_lan_inquiry)
    second_lan = second_lan or \
        raw_input('Please type in the second language of the dict:\n')
    avail_language = ['Spanish', 'English', 'Chinese', 'French', 'German',
                      'Arabic', 'Russian', 'Portuguese', 'Japanese', 'Korean',
                      'Italian', 'Hindi']

    if first_lan not in avail_language or second_lan not in avail_language:
        print notice.UNSUPPORT_LANGUAGE_NOTICE
        pprint.pprint(avail_language)
        get_dict()

    if __doc__:
        __doc__ += '\nThis is a %s_%s_dict made by you and zen_dict system' % \
    ...
    if transfer:
        ...
        current_dict = BaseDict(first_lan, second_lan, agent.raw_dict,
                                agent.score, agent.st_dict, wash_ass=True)
    ...
    if update:
        globals().update({'a': current_dict.a, 'b': current_dict.b,
                          'd': current_dict.d, 't': current_dict.t,
                          'g': current_dict.g, 'sl': current_dict.sl,
                          'm': current_dict.m, 's': current_dict.s,
                          'r': current_dict.r, 'p': current_dict.p,
                          'f': current_dict.f, 'v': current_dict.v,
                          'h': current_dict.h,  'c': current_dict.c,
                          'x': current_dict.x, 'se': current_dict.se,
                          'exam': current_dict.exam, 'save': current_dict.save,
                          'raw_dict': current_dict.raw_dict,
                     })
